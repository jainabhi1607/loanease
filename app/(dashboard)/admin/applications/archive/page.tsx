...
function ApplicationsArchiveContent() {
  ...
  // Helper: check if target_settlement_date is more than 30 days old
  const isOlderThan30Days = (dateStr: string | null) => {
  ...
  const filteredApplications = statusFilteredApplications.filter((app) => {
    ...
    return (
      ...
      formatStatus(app)?.toLowerCase().includes(searchLower)
    );
  });
