...
export default function AddOpportunityPage() {
  ...
  // Load pre-assessment data from sessionStorage if available
  useEffect(() => {
    const preAssessmentData = sessionStorage.getItem('preAssessmentData');
    if (preAssessmentData) {
      try {
        const data = JSON.parse(preAssessmentData);
        ...
        setOpportunityData({
          ...
          assetCountry: data.assetCountry || 'IN',
        });

        // Auto-fill financial details
        setFinancialDetails({
          fundedFromRental: data.fundedFromRental || '',
          proposedRentalIncome: data.proposedRentalIncome || '',
          netProfitBeforeTax: data.netProfitBeforeTax || '',
          amortisation: data.amortisation || '',
          depreciation: data.depreciation || '',
          existingInterestCosts: data.existingInterestCosts || '',
          rentalExpense: data.rentalExpense || '',
          existingLiabilities: data.existingLiabilities || '',
          additionalSecurity: data.additionalSecurity || '',
          smsf: data.smsf || '',
          existingATO: data.existingATO || '',
          creditIssues: data.creditIssues || ''
        });

        // Set calculated values
        if (data.icr) setIcr(data.icr);
        if (data.lvr) setLvr(data.lvr);

        // Set has more info to yes since we have pre-assessment data
        setHasMoreInfo('yes');

        // Clear the sessionStorage after loading
        sessionStorage.removeItem('preAssessmentData');

        toast({
          title: 'Pre-Assessment Data Loaded',
          description: 'Your pre-assessment data has been automatically filled in.',
        });
      } catch (error) {
        console.error('Error loading pre-assessment data:', error);
      }
    }
  }, [toast]);

  // Fetch referrer users on mount and auto-select current user
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.currentUserId) {
            setSelectedUser(data.currentUserId);
          }
        } else {
  ...
  // Fetch clients on mount and when client type changes
  useEffect(() => {
  ...
  const handleSubmit = async (saveAsDraft: boolean = false) => {
    ...
    try {
      ...
      const response = await fetch('/api/referrer/opportunities/create', {
        method: 'POST',
  ...
  return (
    <div className="max-w-[1290px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="space-y-6">
        {/* Referrer User Section */}
        <Card>
          <CardHeader>
            <CardTitle>Referrer User</CardTitle>
          </CardHeader>
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger id="referrer-user">
                  <SelectValue placeholder={isLoadingUsers ? "Loading users..." : "Select"} />
                </SelectTrigger>
                <SelectContent>
