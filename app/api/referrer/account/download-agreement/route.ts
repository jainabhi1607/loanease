...
export async function GET(request: NextRequest) {
  try {
    ...
    const organization = await db.collection(COLLECTIONS.ORGANISATIONS).findOne({
      _id: userData.organisation_id
    });
    ...
    const settings = await db.collection(COLLECTIONS.GLOBAL_SETTINGS)
    ...
    const commencementDateStr = organization.agreement_date || userData.created_at;
    ...
    const approvalDateStr = organization.agreement_date || userData.created_at;
    const approvalDate = new Date(approvalDateStr);
    const agreementIp = organization.agreement_ip || 'Not recorded';
    ...
    if (organization.abn) {
      filename += `-${organization.abn.replace(/\s/g, '')}`;
    }
    filename += '.pdf';
