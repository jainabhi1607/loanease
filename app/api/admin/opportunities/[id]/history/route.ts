...
export async function GET(
  ...
  try {
    ...
    // Type assertion for audit logs
    const typedAuditLogs = auditLogs as unknown as Array<{
      _id: string;
    ...
    const userIds = [...new Set(typedAuditLogs.map(log => log.user_id).filter(Boolean))];
    ...
    const history = typedAuditLogs.map(log => {
  ...
  if (!fieldName && typeof newValue === 'object') {
    ...
    if (newValue.target_settlement_date !== undefined) {
      if (newValue.target_settlement_date) {
        changes.push(`Target Settlement set to ${formatDate(newValue.target_settlement_date)}`);
      } else {
    ...
    if (newValue.date_settled !== undefined) {
      if (newValue.date_settled) {
        changes.push(`Date Settled set to ${formatDate(newValue.date_settled)}`);
      } else {
    ...
    switch (fieldName) {
      ...
      case 'target_settlement':
        if (typeof newValue === 'object' && newValue.target_settlement_date) {
          return `Target Settlement set to ${formatDate(newValue.target_settlement_date)}`;
        }
        return 'Target Settlement Date updated';
      case 'date_settled':
        if (typeof newValue === 'object' && newValue.date_settled) {
          return `Date Settled set to ${formatDate(newValue.date_settled)}`;
        }
