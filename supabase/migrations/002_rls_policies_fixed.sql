...
-- Helper function to get user's role (using public schema)
...
-- Helper function to check if user is admin (Clue Finance)
CREATE OR REPLACE FUNCTION public.is_admin()
RETURNS BOOLEAN AS $$
BEGIN
  RETURN public.get_user_role() IN ('super_admin', 'admin_team');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE POLICY "Users in same organization can view each other"
  ...
  USING (
    ...
    AND public.get_user_role() IN ('referrer_admin', 'referrer_team')
  );
...
CREATE POLICY "Referrer admins can manage team profiles"
  ON user_profiles FOR ALL
  USING (
    public.get_user_role() = 'referrer_admin' 
...
CREATE POLICY "Referrer team can only view their own opportunities"
  ON opportunities FOR SELECT
  USING (
    public.get_user_role() = 'referrer_team' 
    AND created_by = auth.uid()
  );
...
CREATE POLICY "Users can view public comments on their opportunities"
  ...
  USING (
    ...
    AND EXISTS (
      ...
      AND (
        opportunities.organization_id = public.get_user_organization_id() 
        OR public.is_admin()
      )
    )
  );
